...
int OSA_thrCreate(OSA_ThrHndl *hndl, OSA_ThrEntryFunc entryFunc, Uint32 pri, Uint32 stackSize, void *prm)
{
	...
	/*
	status |= pthread_attr_setinheritsched(&thread_attr, PTHREAD_EXPLICIT_SCHED);//å¿…é¡»è??ˆè®¾ç½®ä¸ºä?ç»§æ‰¿çˆ¶çº¿ç¨‹?„è°ƒåº?œºåˆ¶æ‰?½è	status |= pthread_attr_setschedpolicy(&thread_attr, SCHED_FIFO);
	...
	schedprm.sched_priority = OSA_THR_PRI_MAX;//pri;//ä¼˜?ˆçº?	status |= pthread_attr_setschedparam(&thread_attr, &schedprm);
	*/
	if	(status	!=	OSA_SOK)
	{
		OSA_ERROR("OSA_thrCreate() - Could not initialize thread attributes\n");
